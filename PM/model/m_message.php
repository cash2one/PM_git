...
class m_message extends spModel
{
    ...
    */
    ...
    public $msg_type; //$msg_type{0->普通消息,1->项目的消息,2->流程的消息,3->周报，4->PG通知}  0,1,2默认会根据参数断
    public $msg_medal;//$msg_medal 勋章发送通知时候传入勋章ID。
    public $msg_userid;// 勋章获得人的id。

    public function init($msg_context, $proj_id = 0, $pnod_id = 0, $msg_response = 0, $push_type = 1, $push_url = NULL, $sender = NULL, $msg_type = NULL,$msg_medal=0,$msg_userid=0)
    {
    ...
    public function send()
    {
        ...
        if ($this->msg_type == NULL) {
            ...
            elseif ($this->proj_id && !$this->pnod_id) $this->msg_type = 1; else $this->msg_type = 0;
        }
        $msg = array(
            ...
            'msg_medal'=> $this->msg_medal,
            'msg_userid'=> $this->msg_userid
        );
    ...
    public function toProject($isIncPuser = true, $isIncProdUser = true)
    {
        ...
        foreach ($user_rela as $k => $v) {
            array_push($user_array, $v["user_id"]);
        }
    ...
    {
        $userList = spClass("m_user")->findAll();
        $user_array;
        foreach ($userList as $user) {
            $user_array[$user["user_id"]] = $user["user_id"];
        }
