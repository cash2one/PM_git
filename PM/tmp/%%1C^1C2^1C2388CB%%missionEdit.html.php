<?php /* Smarty version 2.6.26, created on 2013-07-12 15:27:34
         compiled from pg/admin/missionEdit.html */ ?>
<?php require_once(SMARTY_CORE_DIR . 'core.load_plugins.php');
smarty_core_load_plugins(array('plugins' => array(array('function', 'spUrl', 'pg/admin/missionEdit.html', 19, false),)), $this); ?>
<!DOCTYPE html>
            ...
            <section class="boxstyle1 top clearfix" style="padding-left: 10px;">
                <table class="table_node" style="width: 680px;padding: 0;">
                    <tr>
                        ...
                        <td><span class="li2">
	      <input name="mtpl-name" type="text" id="mtpl-name" value="<?php echo $this->_tpl_vars['mtplDetails']['mtpl_name']; ?>
" maxlength="45"
                 datatype="Require" msg="任务模板名不能为空" class="itext stitle"/>
                    ...
                    <tr>
					    ...
					    <td><span class="li2">
					      <input type="checkbox" name="is_show" id="is_show" <?php if ($this->_tpl_vars['mtplDetails']['is_show'] == 1): ?>checked<?php endif; ?>>
					    </span></td>
					  </tr>
                    <tr>
...
"><?php echo $this->_tpl_vars['rs']; ?>
                    ...
                    <tr>
                        ...
                        <td class="flow-box">
                            <?php $_from = $this->_tpl_vars['mtplDetails']['flow_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }if (count($_from)):
    foreach ($_from as $this->_tpl_vars['rs']):
?>
                            <div class="li2 flow-item">
             流程名：               <input type="text"  maxlength="45" class="itext stitle flow-name" value="<?php echo $this->_tpl_vars['rs']['flow_name']; ?>
"/>
<span class="beforePnode_add beforePnode_set beforeflow_set" data_id="<?php echo $this->_tpl_vars['rs']['flow_id']; ?>
" data_name="<?php echo $this->_tpl_vars['rs']['flow_name']; ?>
">
				    		前置流程设置
</span>
<br>
                                流程类型：
                                <input type="hidden" class="flow-id" value="<?php echo $this->_tpl_vars['rs']['flow_id']; ?>
">
...
"><?php echo $this->_tpl_vars['irs']; ?>
                 ...
                 <span class="flow-item-one flow-item-outcome">
                    ...
                    <tr>
                        ...
                        <td>贡献值：<input class="itext" id="mtpl-gx" value="<?php echo $this->_tpl_vars['mtplDetails']['mtpl_gx']; ?>
"></span> </td>
                    </tr>
                    <tr>
              ...
              <textarea class="itext" style="resize: none;width: 436px;line-height: 24px;padding: .5em;" rows="5"
                        id="mtpl-desc"><?php echo $this->_tpl_vars['mtplDetails']['mtpl_desc']; ?>
</textarea>
          </span></td>
                    </tr>
                </table>
            ...
            <section class="boxstyle2 bottom">
                <input id="submit-btn" type="button" value="提交" class="btn btn_main1"/> <a
                    href="<?php echo $this->_plugins['function']['spUrl'][0][0]->__template_spUrl(array('c' => 'pgadmin','a' => 'missionlist'), $this);?>
...
unset($_smarty_tpl_vars);
 ?>
<div id="flow_before_popwin" class="popwin">
...
<script type="text/template" id="flow-item-tpl">
    <div class="li2 flow-item">
流程名：<input type="text"  maxlength="45" class="itext stitle flow-name"/>
<br>
        流程类型：
        <select class="flow-type1"><?php $_from = $this->_tpl_vars['nodecls']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }if (count($_from)):
    foreach ($_from as $this->_tpl_vars['k'] => $this->_tpl_vars['rs']):
...
<script type="text/javascript">
    $(function () {
        ...
        var jsonDeatails = '<?php echo $this->_tpl_vars['json_mtpldtls']; ?>
';
        //数据初始化渲染
        var renderInit = (function () {
            var proj1Lv = '<?php echo $this->_tpl_vars['mtplDetails']['mtpl_proj1lv']; ?>
';
            var proj2Lv = '<?php echo $this->_tpl_vars['mtplDetails']['mtpl_proj2lv']; ?>
';
            renderDays($('select.flow-start-time'), 30);
            renderDays($('select.flow-end-time'), 30);
            //获取该模板的流程
            var flowList = JSON.parse('<?php echo $this->_tpl_vars['json_mtpldtls']; ?>
')['flow_list'];
            console.log(flowList);
            var flowItems = $('.flow-item');
            for (var i = 0; i < flowList.length; i++) {
                var currentEle= flowItems.eq(i);
                currentEle.find('input.flow-name').val(flowList[i]['flow_name']);
...
<?php if (($this->_foreach['mycountb']['iteration'] == $this->_foreach['mycountb']['total']) == 1): ?><?php else: ?>,<?php endif; ?><?php endforeach; endif; unset($_from); ?>'.split(',');
		...
		//前置流程
        $(".beforeflow_set").click(function() {
			flow_before_pnode(<?php echo $this->_tpl_vars['mtplDetails']['mtpl_id']; ?>
,$(this).attr('data_id'),$(this).attr('data_name'));
		});
        // 删除流程
        ...
        $("#submit-btn").click(function () {
        	if(!confirm('确定修改？\n修改将会解除所有的流程关联，需要再次来此关联流程哦。'))
        	{
        		return;
        	}
        	var bb = $('#is_show:checked');
        	var is_show=bb.length;
            var url = '<?php echo $this->_plugins['function']['spUrl'][0][0]->__template_spUrl(array('c' => 'pgadmin','a' => 'mtplupdate_do'), $this);?>
';
            ...
            $.ajax({
                url:url,
                type:'POST',
                data:'mtplid=<?php echo $this->_tpl_vars['mtplId']; ?>
&mtplName='+mtplName+'&is_show='+is_show+'&mtplLv='+mtplLv+'&mtplProj1lv='+mtplProj1lv+'&mtplProj2lv='+mtplProj2lv+'&mtplGx='+mtplGx+'&mtplDesc='+mtplDesc+"&mtplFlowArr="+JSON.stringify(mtplFlow),
                success:function (data) {
...
';
    ...
    function flow_before_pnode(mtpl_id,flow_id,flow_name)
    {
		var url= "<?php echo $this->_plugins['function']['spUrl'][0][0]->__template_spUrl(array('c' => 'pgadmin','a' => 'flow_before'), $this);?>
" + "&flow_id="+flow_id + "&mtpl_id="+mtpl_id+ "&flow_name="+flow_name;
        $('#flow_before_popwin div').load(url,function(){_$.popWin("flow_before_popwin");});
    }
</script>
</body>
</html>